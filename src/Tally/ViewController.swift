...
extension ViewController: WKScriptMessageHandler {
  func userContentController(_ userContentController: WKUserContentController, didReceive message: WKScriptMessage) {
        ...
        if message.name == "google-signin" {
            handleGoogleSignIn()
        }
  }
}

// ===== NATIVE SIGN IN WITH GOOGLE (added 14 Aug 2026) =====
//
// WHY THIS EXISTS, and why three earlier attempts to fix Google sign-in in the
// web view could never have worked: GOOGLE REFUSES OAUTH FROM EMBEDDED WEB
// VIEWS. Its policy names WKWebView explicitly and has been enforced since
// 30 September 2021. No allow-list entry, no origin list, no Info.plist key
// makes an embedded web view acceptable to Google — it is a deliberate block,
// not a misconfiguration. Builds 17 and 18 chased the symptoms (a 400 from
// accounts.youtube.com, then a silent return to the welcome screen) of a flow
// that was refused at the door.
//
// The documented answer, which is also what every mature wrapper framework
// does, is the shape already used here for Apple:
//     native SDK signs in  ->  hand the ID token to the Firebase JS SDK
// The web layer then calls signInWithCredential and is properly signed in.
//
// SIMPLER THAN THE APPLE BRIDGE: no nonce. Apple requires the SHA-256 of a
// nonce generated by the web app; Google does not, so the token can be fetched
// entirely on this side and posted back.
//
// The client ID comes from GIDClientID in Info.plist, which GIDSignIn reads by
// itself. The callback returns through the REVERSED_CLIENT_ID URL scheme and is
// claimed in SceneDelegate BEFORE the generic handler there rewrites it.
extension ViewController {

    func handleGoogleSignIn() {
        guard let clientId = Bundle.main.object(forInfoDictionaryKey: "GIDClientID") as? String,
              !clientId.isEmpty else {
            sendGoogleSignInResult(["error": "no-client-id"])
            return
        }
        ...
        DispatchQueue.main.async {
            GIDSignIn.sharedInstance.signIn(withPresenting: self) { [weak self] result, error in
                ...
                if let error = error as NSError? {
                    // A cancel is a normal outcome, not a failure to shout about —
                    // the web app maps it to auth/popup-closed-by-user and stays quiet.
                    if error.code == GIDSignInError.canceled.rawValue {
                        self.sendGoogleSignInResult(["error": "cancelled"])
                    } else {
                        // Carry Google's own description through. There is no device
                        // console without a Mac, so this is the only way a failure
                        // here can ever be diagnosed.
                        self.sendGoogleSignInResult([
                            "error": "failed",
                ...
                guard let user = result?.user,
                    ...
                    self.sendGoogleSignInResult(["error": "no-identity-token"])
                    return
                }
                ...
                // Google returns the name and email on every sign-in, unlike Apple
                // which gives them only on the first authorisation. Passed through
                // so the welcome screen's name step can arrive prefilled.
                ...
                self.sendGoogleSignInResult(payload)
            }
        }
    }

    // Mirrors sendAppleSignInResult exactly, including the base64 hop, so the two
    // bridges behave identically on the web side.
    func sendGoogleSignInResult(_ payload: [String: Any]) {
        ...
        DispatchQueue.main.async {
            Tally.webView.evaluateJavaScript(
                "window.dispatchEvent(new CustomEvent('google-signin-result',{detail:'\(encoded)'}))"
            )
        }
    }
}
